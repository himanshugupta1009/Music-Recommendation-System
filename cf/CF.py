...
# publication.
# modifed to python3 by Jaeyoung Dec 2019 for the course project CSCI5502
# arguments input
...
def main(argv):
    if len(argv)<3:
        print("Nee more arguments, Example:MSD_subm_rec.py user_min user_max resultFile.txt")
        user_min = 1
        user_max = 110000
...
if __name__ == "__main__":
    ...
    userIDs = list(range(0, 110000))
    comb = combinations(userIDs, 2)
    for i in list(comb):
        main([i[0], i[1], str(i[0])+"_"+str(i[1])+".txt"])
